...
export async function main(client: Client) {
  ...
  logger.legacy(
    "[*] iHorizon Discord Bot (https://github.com/ihrz/ihrz).".gray,
  );
  logger.legacy(
  ...
  let handlerFiles = readdirSync(handlerPath).filter((file) =>
    file.endsWith(".js"),
  );
...
export const getConfig = (): ConfigData => {
  if (!global_config) {
    throw new Error(
      "Configuration file has not been initialized. Call initConfig first.",
    );
  }
...
export function timestampInitializer() {
  ...
  logger.log(
    `${global_config.console.emojis.OK} >> Timestamp Generated in .uptime`,
  );
}
